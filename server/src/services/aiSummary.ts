...
export const generateAiSummary = async (input: IGenerationInput): Promise<{ summary: string; fallbackUsed: boolean }> => {
  ...
  // Build the prompt body matching PROMPTS.md spec
  ...
  const prompt = `You are WiseBill AI, a premium financial optimization assistant for SaaS stacks.
...
5. Do NOT output any conversational preambles, greetings, markdown headings, or bullet points. Output ONLY the raw paragraph text.`;
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (text && text.trim().length > 0) {
          return { summary: text.trim(), fallbackUsed: false };
        }
      }
      console.warn(`Gemini API returned status ${response.status}. Trying Anthropic.`);
    } catch (err) {
  ...
  if (process.env.ANTHROPIC_API_KEY) {
    try {
      ...
      console.warn(`Anthropic API returned status ${response.status}. Trying fallback.`);
    } catch (err) {
      console.error('Error invoking Anthropic API:', err);
    }
  }
  ...
  if (process.env.OPENAI_API_KEY) {
    try {
      ...
      console.warn(`OpenAI API returned status ${response.status}. Trying fallback.`);
    } catch (err) {
      console.error('Error invoking OpenAI API:', err);
    }
  }

  // 3. Fallback: Generate a high-quality, personalized template summary
